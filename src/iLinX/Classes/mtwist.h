...
#ifndef MT_EXTERN
//#ifdef __cplusplus
#define MT_EXTERN static			/* C++ doesn't need static */
//#else /* __cplusplus */
...
MT_EXTERN unsigned long	mts_lrand(mt_state* state);
					/* Generate 32-bit value, any gen. */
#ifndef MT_NO_LONGLONG
MT_EXTERN unsigned long long
			mts_llrand(mt_state* state);
					/* Generate 64-bit value, any gen. */
#endif /* MT_NO_LONGLONG */
MT_EXTERN double		mts_drand(mt_state* state);
					/* Generate floating value, any gen. */
					/* Fast, with only 32-bit precision */
MT_EXTERN double		mts_ldrand(mt_state* state);
					/* Generate floating value, any gen. */
					/* Slower, with 64-bit precision */

MT_EXTERN unsigned long	mt_lrand(void);	/* Generate 32-bit random value */
#ifndef MT_NO_LONGLONG
MT_EXTERN unsigned long long
			mt_llrand(void);
					/* Generate 64-bit random value */
#endif /* MT_NO_LONGLONG */
MT_EXTERN double		mt_drand(void);
					/* Generate floating value */
					/* Fast, with only 32-bit precision */
MT_EXTERN double		mt_ldrand(void);
					/* Generate floating value */
					/* Slower, with 64-bit precision */
