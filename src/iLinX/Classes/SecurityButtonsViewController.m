//
//  SecurityButtonsViewController.m
//  iLinX
//
...
@interface SecurityButtonsViewController ()
...
@implementation SecurityButtonsViewController
...
- (void) service: (NLServiceSecurity *) service changed: (NSUInteger) changed
{
  if ((changed & SERVICE_SECURITY_MODE_TITLES_CHANGED) != 0 && 
      _buttonCount != [_securityService buttonCountInControlMode: _controlMode])
  {
    ...
    [self initialisePages];
    _visiblePage = [_pageControllers objectAtIndex: 0];
    [_visiblePage viewWillAppear: NO];
    [_visiblePage viewDidAppear: NO];
  }
}
...
- (void) loadScrollViewForPage: (NSUInteger) page
{
  if (page < _pager.numberOfPages)
  {
    ...
    SecurityButtonsPageViewController *controller = [_pageControllers objectAtIndex: page];
    
    if ((NSNull *) controller == [NSNull null])
    {
      controller = [[SecurityButtonsPageViewController alloc]
                    initWithService: _securityService controlMode: _controlMode 
